import sys

# List of all screen IDs (from database query)
screen_ids = [
    86,87,88,89,90,91,92,93,94,95,96,97,98,99,100,101,102,103,
    117,118,119,120,121,122,123,124,125,126,127,128,129,130,131,132,133,134,
    148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,
    179,180,181,182,183,184,185,186,187,188,189,190,191,192,193,194,195,196,197,198,199,200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,
    242,243,244,245,246,247,248,249,250,251,252,253,254,255,256,257,258,259,260,261,262,263,264,265,266,267,268,269,270,271,272,273,274,275,276,277,278,279,
    305,306,307,308,309,310,311,312,313,314,315,316,317,318,319,320,321,322,323,324,325,326,327,328,329,330,331,332,333,334,335,336,337,338,339,340,341,342,
    368,369,370,371,372,373,374,375,376,377,378,379,380,381,382,383,384,385,386,387,388,389,390,391,392,393,394,395,396,397,398,399,400,401,402,403,404,405,406,407,408,409,410,411,412,413,414,415,416,417,418,419,420,421,422,423,424,425,
    431,432,433,434,435,436,437,438,439,440,441,442,443,444,445,446,447,448,449,450,451,452,453,454,455,456,457,458,459,460,461,462,463,464,465,466,467,468,469,470,471,472,473,474,475,476,477,478,479,480,481,482,483,484,485,486,487,488,
    494,495,496,497,498,499,500,501,502,503,504,505,506,507,508,509,510,511,512,513,514,515,516,517,518,519,520,521,522,523,524,525,526,527,528,529,530,531,532,533,534,535,536,537,538,539,540,541,542,543,544,545,546,547,548,549,550,551,
    552,553
]

print("-- Generated SQL to add 150 seats to ALL 468 screens")
print("-- Total: 70,200 seats (468 screens × 150 seats)")
print("-- Each screen: 45 Premium (A-C), 60 Regular (D-G), 45 Economy (H-J)")
print("")
print("DELETE FROM seats;")
print("")

# Generate INSERT statements for each screen
for screen_id in screen_ids:
    print(f"-- Screen {screen_id}")
    print(f"INSERT INTO seats (screen_id, `row_number`, seat_number, seat_type) VALUES")
    
    values = []
    
    # Premium seats (Rows A-C)
    for row in ['A', 'B', 'C']:
        for seat_num in range(1, 16):
            values.append(f"({screen_id},'{row}',{seat_num},'PREMIUM')")
    
    # Regular seats (Rows D-G)
    for row in ['D', 'E', 'F', 'G']:
        for seat_num in range(1, 16):
            values.append(f"({screen_id},'{row}',{seat_num},'REGULAR')")
    
    # Economy seats (Rows H-J)
    for row in ['H', 'I', 'J']:
        for seat_num in range(1, 16):
            values.append(f"({screen_id},'{row}',{seat_num},'ECONOMY')")
    
    # Print values in chunks of 50 to keep lines reasonable
    for i in range(0, len(values), 50):
        chunk = values[i:i+50]
        if i + 50 >= len(values):
            print(",".join(chunk) + ";")
        else:
            print(",".join(chunk) + ",")
    
    print("")

print("-- Verify total seats added")
print("SELECT COUNT(*) as total_seats, COUNT(DISTINCT screen_id) as total_screens FROM seats;")
print("SELECT seat_type, COUNT(*) as count FROM seats GROUP BY seat_type;")
