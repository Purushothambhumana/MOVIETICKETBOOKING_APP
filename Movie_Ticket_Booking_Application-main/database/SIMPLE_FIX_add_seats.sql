-- ========================================
-- SIMPLE FIX: Add Seats to All Screens
-- ========================================
-- Execute this script in MySQL Workbench or any MySQL client
-- This will add 150 seats to each screen in your database

USE antipromovie;

-- Step 1: Clear existing seats (optional, remove if you want to keep existing seats)
DELETE FROM seats;

-- Step 2: Add seats to ALL screens
-- This uses a simple approach: for each screen, insert 150 seats

-- Get all screen IDs and add seats
...
UNION ALL
SELECT 
    'Seats per screen (should be 150)',
    COUNT(*) / (SELECT COUNT(*) FROM screens)
FROM seats;

-- Show breakdown by screen
SELECT 
    sc.id as screen_id,
    t.name as theatre_name,
    sc.screen_number,
    COUNT(st.id) as total_seats,
    SUM(CASE WHEN st.seat_type = 'PREMIUM' THEN 1 ELSE 0 END) as premium,
    SUM(CASE WHEN st.seat_type = 'REGULAR' THEN 1 ELSE 0 END) as regular,
    SUM(CASE WHEN st.seat_type = 'ECONOMY' THEN 1 ELSE 0 END) as economy
FROM screens sc
